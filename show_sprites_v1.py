...
class PygameImageViewer:
    def __init__(self, image_paths, cols=16, thumb_size=128, padding=10):
        """
        ...
        self.rows = (len(self.images) + cols - 1) // cols
        self.window_width = cols * (thumb_size + padding) + padding + self.offsetW
    ...
    def update_visible_range(self):
        ...
        self.first_visible_row = max(0, self.scroll_y // (self.thumb_size + self.padding))
        visible_rows = self.window_height // (self.thumb_size + self.padding) + 2
        self.last_visible_row = min(self.rows, self.first_visible_row + visible_rows)
    ...
    def run(self):
        ...
        while running:
                ...
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_ESCAPE:
                        ...
                        if old_scroll != self.scroll_y:
                            self.calculate_positions()  # Обновляем позиции при скролле
                            self.need_redraw = True
            ...
            if self.need_redraw:
                ...
                # Информация внизу
                # font = pygame.font.Font(None, 24)
                # info_text = f"Images: {len(self.images)} | Scroll: ↑↓ or wheel | Click to view | ESC: exit"
                # if self.hovered.current is not None:
                #     info_text += f" | Hover: {self.hovered.current+1}"
                # if self.selected.current is not None:
                #     info_text += f" | Selected: {self.selected.current+1}"
                
                # text = font.render(info_text, True, (200, 200, 200))
                # text_rect = text.get_rect(center=(self.window_width//2, self.window_height-20))
...
if __name__ == "__main__":
    ...
    if not image_files:
        ...
        viewer = PygameImageViewer(image_files, cols=16, thumb_size=96, padding=10)
        viewer.run()