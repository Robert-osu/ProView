...
class ProgrammatorViewer:
    def __init__(self, image_paths, pro: Programmator, k_size=1):
        ...
        self.panel_height = int(32 * self.k_size)  # Высота панели
        ...
        self.max_scroll = max(0, self.rows * (self.thumb_size + self.padding) + 
                     self.padding - (self.window_height - self.offsetH))
        ...
        # 2. Создаем фасад и привязываем команды
    ...
    def update_from_pro(self):
        ...
        print(f"Программа обновлена. Команд: {len(self.cmd_list)}, строк: {self.rows}")
    ...
    def _setup_key_facade(self):
        ...
        # self.key_facade.bind_mod_key(pygame.KMOD_CTRL, pygame.K_c, self.copy_clipboard_cmd)
        # self.key_facade.bind_mod_key(pygame.KMOD_CTRL, pygame.K_v, self.paste_clipboard_cmd)
        self.key_facade.bind_scan_code(25, pygame.KMOD_CTRL, self.paste_clipboard_cmd)
        
        # Можно также использовать bind_combo для удобства
        # self.key_facade.bind_combo('ctrl+v', PasteCommand(self))
        # self.key_facade.bind_combo('ctrl+s', SaveCommand(self))
    
    def update_key_bindings_from_dict(self):
        """Обновление привязок из словаря key_bindings (для совместимости)"""
        # Очищаем текущие привязки
        self.key_facade.key_bindings.clear()
        self.key_facade.mod_bindings.clear()
        
        # Перенастраиваем из словаря
        mapping = {
            'ВВЕРХ': pygame.K_UP,
            'ВНИЗ': pygame.K_DOWN,
            'ВЛЕВО': pygame.K_LEFT,
        ...
        for action, key in self.key_bindings.items():
            if action == 'ВВЕРХ':
                self.key_facade.bind_key(key, self.scroll_up_cmd)
            elif action == 'ВНИЗ':
                self.key_facade.bind_key(key, self.scroll_down_cmd)
            elif action == 'ДЕЙСТВИЕ':
                self.key_facade.bind_key(key, self.select_cmd)
            elif action == 'МЕНЮ':
                self.key_facade.bind_key(key, self.open_settings_cmd)

    def create_menu_button(self):
        """Создание нескольких кнопок на панели"""
        button_width = int(100 * self.k_size)
        button_height = int(30 * self.k_size)
        button_margin = int(10 * self.k_size)

        # Размер шрифта в зависимости от k_size
        font_size = int(24 * self.k_size)   
        ...
        btn1 = pygame_gui.elements.UIButton(
            relative_rect=pygame.Rect(
                self.padding, 
                (self.panel_height - button_height) // 2, 
                button_width, 
                button_height
            ),
            text="Меню",
        ...
        btn2 = pygame_gui.elements.UIButton(
            relative_rect=pygame.Rect(
                self.padding + button_width + button_margin, 
                (self.panel_height - button_height) // 2, 
                button_width, 
                button_height
            ),
        ...
        btn3 = pygame_gui.elements.UIButton(
            relative_rect=pygame.Rect(
                self.padding + (button_width + button_margin) * 2, 
                (self.panel_height - button_height) // 2, 
                button_width, 
                button_height
            ),
    ...
    def draw_top_panel(self):
        ...
        # Рисуем фон панели
        panel_rect = pygame.Rect(0, 0, self.window_width, self.panel_height)
        ...
        pygame.draw.line(self.screen, (100, 100, 110), 
                        (0, self.panel_height), 
                        (self.window_width, self.panel_height), 2)
        
        # Рисуем дополнительную информацию на панели
        font = pygame.font.Font(None, int(20 * self.k_size))
        
        # Информация о количестве команд
        text = font.render(f"Привет от Majin'а", True, (200, 200, 200))
        text_x = self.window_width - text.get_width() - self.padding * 2
        text_y = (self.panel_height - text.get_height()) // 2
        self.screen.blit(text, (text_x, text_y))
        
        # Рисуем кнопку (она будет отрисована через ui_manager)
    ...
    def copy_to_clipboard(self):
        ...
        print("Метод copy_to_clipboard вызван!")
        try:
            pygame.scrap.put_text(pro.getEncode()) #TODO
    ...
    def paste_from_clipboard(self):
        ...
        print("Метод paste_from_clipboard вызван!")
        try:
            # Получаем текст из буфера обмена
            clipboard_text = pygame.scrap.get_text()
            if clipboard_text:
                print(f"Текст из буфера: {clipboard_text}")
                self.pro.copy(clipboard_text)
                self.update_from_pro()
                self.need_redraw = True
            else:
                print("Буфер обмена пуст или содержит не текст")
        except Exception as e:
    ...
    def on_menu_click(self):
        ...
        # Здесь можно открывать меню или выполнять другие действия
        self.open_settings()  # Например, открываем настройки
    ...
    def run(self):
        ...
        while running:
            ...
            for event in pygame.event.get():
                ...
                # ===== ИСПОЛЬЗУЕМ ФАСАД ДЛЯ ОБРАБОТКИ КЛАВИШ =====
                elif event.type == pygame.KEYDOWN or event.type == pygame.KEYUP:
                    # Передаем событие в фасад
            ...
            if self.menu_need_redraw:
                # Рисуем UI поверх всего
                self.ui_manager.draw_ui(self.screen)

                pygame.display.flip()
                self.menu_need_redraw = False
    ...
    def check_hover(self):
        ...
        if mouse_y < self.panel_height:
            # Если мышь над панелью, запрашиваем перерисовку для подсветки кнопок
            self.menu_need_redraw = True
            return None
        ...
        x = self.padding + col * (self.thumb_size + self.padding) + self.offsetW
        y = self.padding + row * (self.thumb_size + self.padding) - self.scroll_y - self.offsetW
...
if __name__ == "__main__":
    ...
    if not image_files:
        ...
        viewer = ProgrammatorViewer(image_files, pro, k_size=v_k[1])
        viewer.run()
